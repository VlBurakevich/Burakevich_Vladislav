...
public class DiscountService {
    ...
    public DiscountListDto getDiscounts(Integer page, Integer size) {
        Page<Discount> discountPage = discountRepository.findAll(PageRequest.of(page, size));

        List<DiscountDto> discounts = discountPage.getContent().stream()
                .map(discountMapper::entityToDto)
                .toList();

        return new DiscountListDto(discounts);
    }
